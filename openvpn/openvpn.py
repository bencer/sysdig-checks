'''
OpenVPN check for Sysdig Monitor
lsocket and manager classes taken from https://github.com/ajenti/ajenti/blob/1.x/ajenti/plugins/openvpn/manager.py
'''
...
class OpenVPN(AgentCheck):
    def check(self, instance):
        ...
        ### FIXME add support for unix socket path and entering into container namespace
        ...
        vpn = manager(host=domain, port=port, password=password, timeout=timeout)

        connect_attempts = 0
        while connect_attempts < 5 and not vpn.connected:
            try:
                vpn.connect()
            except socket.timeout:
                self.log.error("OpenVPN check: connection timed out")
                connect_attempts += 1
                self.gauge('openvpn.test', -1, tags=tags) # return -1 on timeout
                time.sleep(timeout)
            except Exception as e:
                self.log.error("OpenVPN check: something failed: %s" % e)
                connect_attempts += 1
                self.gauge('openvpn.test', -2, tags=tags) # return -2 on any other error
                time.sleep(timeout)
        if vpn.connected:
            stats = vpn.stats()
            self.gauge('openvpn.test', 0, tags=tags)
        else:
            self.log.error("OpenVPN check: couldn't connect to OpenVPN admin port")
            self.gauge('openvpn.test', -2, tags=tags)
        for k, v in stats.items():
            name = k
            value = float(v)
            self.gauge('openvpn.' + name, value, tags=tags)
